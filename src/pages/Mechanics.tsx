...
import { Loader2, Wrench, DollarSign, Target, TrendingUp, TrendingDown, AlertTriangle, User } from 'lucide-react'
...
export function Mechanics() {
  ...
  async function fetchData() {
    try {
      const [ordersRes, goalsRes] = await Promise.all([
        supabase
          .from('ordens_servico')
          .select('mecanico, valor_total, custo_total, margem_lucro, status, data_abertura')
          .not('mecanico', 'is', null),
        supabase
  ...
  filteredOrders.forEach(o => {
    ...
    if (o.status === 'fechada') {
      ...
      m.custoTotal += Number(o.custo_total || 0)
    } else {
      m.osAbertas++
    }
    if (o.margem_lucro) m.margens.push(Number(o.margem_lucro))
  })

  const mechanics = Object.values(mechanicsMap).map((m: any) => ({
    ...m,
    ticketMedio: m.osFechadas > 0 ? m.faturamento / m.osFechadas : 0,
    margemMedia: m.margens.length > 0 ? m.margens.reduce((a: number, b: number) => a + b, 0) / m.margens.length : 0,
    lucro: m.faturamento - m.custoTotal,
  })).sort((a, b) => b.faturamento - a.faturamento)

  const totalOS = mechanics.reduce((s, m) => s + m.totalOS, 0)
  const totalFaturamento = mechanics.reduce((s, m) => s + m.faturamento, 0)
  const totalOsFechadas = mechanics.reduce((s, m) => s + m.osFechadas, 0)
  const ticketMedioGeral = totalOsFechadas > 0 ? totalFaturamento / totalOsFechadas : 0
  const margemMediaGeral = mechanics.length > 0
    ? mechanics.reduce((s, m) => s + m.margemMedia, 0) / mechanics.length : 0
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <main className="flex-1 overflow-x-hidden overflow-y-auto p-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-slate-800 p-5 rounded-2xl border border-slate-700">
            <p className="text-slate-400 text-xs font-semibold uppercase tracking-wider flex items-center gap-1">
              <Wrench className="w-3.5 h-3.5" /> OS no Período
            </p>
            <h3 className="text-3xl font-bold text-white mt-1">{totalOS}</h3>
            <p className="text-slate-500 text-xs mt-1">ordens de serviço</p>
          </div>
          <div className="bg-slate-800 p-5 rounded-2xl border border-slate-700">
            <p className="text-slate-400 text-xs font-semibold uppercase tracking-wider flex items-center gap-1">
        ...
        <div className="bg-slate-800 rounded-2xl border border-slate-700 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-700 flex items-center justify-between">
            ...
            <span className="text-xs text-slate-500">{mechanics.length} mecânicos</span>
          </div>
            ...
            <table className="w-full text-left text-sm text-slate-400">
              ...
              <tbody className="divide-y divide-slate-700/50">
                {loading ? (
                  ...
                  mechanics.map((mec, index) => {
                    const margemStyle = getMargemStyle(mec.margemMedia)
                    const MargemIcon = margemStyle.icon
                    return (
                      <tr key={mec.nome} className="hover:bg-slate-700/30 transition-colors">
                        ...
                        <td className="px-6 py-4 text-center font-medium text-white">{mec.totalOS}</td>
                        ...
                        <td className="px-6 py-4 text-right font-medium text-white">{fmt(mec.faturamento)}</td>
                        <td className="px-6 py-4 text-right font-medium text-emerald-400">{fmt(mec.lucro)}</td>
                        <td className="px-6 py-4 text-right font-medium text-purple-400">{fmt(mec.ticketMedio)}</td>
                        <td className="px-6 py-4 text-center">
                          <div className={`flex items-center justify-center gap-1.5 ${margemStyle.color}`}>
                            <MargemIcon className="w-4 h-4" />
                            <span className="font-bold">{mec.margemMedia.toFixed(1)}%</span>
        ...
        {mechanics.filter(m => m.margemMedia < minMargin).length > 0 && (
          <div className="mt-6 bg-rose-500/5 border border-rose-500/20 rounded-2xl p-5">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="text-rose-400 font-semibold">Atenção: Mecânicos Abaixo da Meta de Margem ({minMargin}%)</h3>
            </div>
