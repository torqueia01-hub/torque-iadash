...
export function Retention() {
  ...
  async function fetchClientes() {
    try {
      const { data } = await supabase
        .from('clientes')
        .select('*')
        .gte('dias_sem_retorno', 90) // Traz apenas clientes sumidos há 3 meses ou mais
        .eq('ativo', true)
  ...
  // Inteligência de Status baseada no tempo
  ...
  const getWhatsAppLink = (phone: string, nome: string, dias: number) => {
    ...
    if (!cleaned) return '#' // Se não tem número, não faz nada
    
    // Coloca o 55 do Brasil automaticamente se estiver faltando
    const withCountry = cleaned.startsWith('55') && cleaned.length >= 12 ? cleaned : `55${cleaned}`
    
    // Mensagem inteligente que já vai preenchida no WhatsApp
  ...
  // Filtra os clientes pela barra de pesquisa
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <header className="h-16 border-b border-slate-800 flex items-center justify-between px-8 bg-slate-900/50">
        ...
        <button className="bg-slate-800 border border-slate-700 hover:bg-slate-700 text-white px-4 py-2 rounded-lg font-medium flex items-center space-x-2 transition-colors">
      ...
      <main className="flex-1 overflow-x-hidden overflow-y-auto p-8">
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-6">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full bg-slate-800 border border-slate-700 text-white rounded-lg pl-10 pr-4 py-2.5 focus:outline-none focus:border-purple-500 transition-all placeholder:text-slate-500"
            />
          </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-64 bg-slate-800 rounded-2xl border border-slate-700 border-dashed">
            ...
            <p className="text-slate-400">Você contatou todas as oportunidades de retenção.</p>
          </div>
        ) : (
              ...
              return (
                <div key={item.id} className="bg-slate-800 rounded-2xl border border-slate-700 p-6 flex flex-col hover:border-slate-600 transition-colors shadow-sm">
                  ...
                  >
                    ...
                    <span>{whatsappLink !== '#' ? 'Enviar Oferta via WhatsApp' : 'Sem número cadastrado'}</span>
                  </a>
                </div>
              )
            })}
